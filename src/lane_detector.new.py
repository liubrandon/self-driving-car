...
def detect(org_image):
    ...
    #Gamma correction
    img = org_image
    img = adjust_gamma(img, .4) #originally .17
    ...
    cv2.imshow('4 after blur', img)

    #thresholding
    after_thresholding = cv2.adaptiveThreshold(img, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 37, 6) # TODO: Dynamic threshold
    ...
    max_line_gap = 100      #The maximum gap between two points to be considered in the same line.
